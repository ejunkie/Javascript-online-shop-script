...
function EJ_Shop(params, callback){
	...
	this.hidden_filters = params.hidden_filters ? params.hidden_filters : []
	...
	this.refreshListings = function(){
		...
		//hide the products which belong to the hidden filters incase no filter has been choosen
		if(self.hidden_filters && self.filters.length == 0){

			var tmp = []
			self.hidden_filters.forEach(function(y){
				if(y && y != "" & y != "null" & y != "NULL")
				tmp.push(y.toUpperCase())
			})
			self.hidden_filters = tmp

			
			if(self.filters){
				var temp_hidden_filters = []
				self.hidden_filters.forEach(function(x){
					if(self.filters.indexOf(x) == -1) temp_hidden_filters.push(x)				
				})
			}
			...
			self.products.forEach(function(x){
				if(x.tags){
					var counter = 0;
					x.tags.forEach(function(y){
						if(temp_hidden_filters.indexOf(y.toUpperCase()) != -1)
							counter++
					})
					if(counter == 0) tmpArr.push(x)
				}
			})
			self.products = tmpArr
		}
